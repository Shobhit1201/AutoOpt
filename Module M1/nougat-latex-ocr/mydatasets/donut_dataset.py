...
class NougatDataset(Dataset):
    def __init__(
            ...
            max_length: int,
            phase: str = "train",
            max_dimensions=(1224, 1020), min_dimensions=(32, 32),
            **kwargs
    ):
        super().__init__()
...
class NougatPadFixSizeCollectFn(object):
    ...
    def __call__(self, instances):
        batch = dict(labels=[], mask=[], processed_parse=[])
        img_batch = []
        ...
        batch['labels'] = pad_sequence(
            [torch.LongTensor(x) for x in batch['labels']], batch_first=True, padding_value=self.pad_token_id
        )
        batch['mask'] = pad_sequence(
            [torch.LongTensor(x) for x in batch['mask']], batch_first=True, padding_value=0
        )

        # Process images using your fixed-size processor
        pixel_values = []
        for i, img in enumerate(img_batch):
            if self.debug:
                cache_path = os.path.join(CACHE_ROOT, f"{i}.jpg")
                img.save(cache_path)
            processed = self.processor(img, return_tensors="pt").pixel_values
            pixel_values.append(processed)

        # Concatenate processed images into a batch
