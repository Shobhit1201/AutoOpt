...
from metrics import AverageMeter, TokenAccMetric, get_metrics
...
from transformers import AutoTokenizer, VisionEncoderDecoderModel, VisionEncoderDecoderConfig, NougatProcessor, MBartForCausalLM
from nougat_latex.image_processing_nougat import NougatImageProcessor
from nougat_latex.image_processor_img2latex import NougatLaTexProcessor
from .hybrid_encoder_swin import HybridResNetSwinEncoder
...
class DonutExperiment(BaseExperiment):
    ...
    def evaluate(self, **kwargs):
        batch_edit_distance = AverageMeter()
        ...
        for i, batch_data in tqdm(enumerate(self.eval_data_loader), total=len(self.eval_data_loader)):
            ...
            for ans_seq, pred_seq in zip(answers, pred_sequences):
                pred_seq = pred_seq.replace(self.processor.tokenizer.bos_token, "") \
                                  ...
                                  .replace(self.processor.tokenizer.pad_token, "").strip()
                ans_seq = ans_seq.strip()

                if len(ans_seq) > 0:
                    batch_edit_distance.update(distance(pred_seq, ans_seq) / len(ans_seq))
                    all_preds.append(pred_seq)
                    all_refs.append(ans_seq)

        # Compute full OCR metrics
        ocr_metrics = get_metrics(all_refs, all_preds)

        logger.info("evaluating...")
        logger.info("token_acc: {:.4f}; batch_edit_dis: {:.4f}".format(eval_metric.mean(), batch_edit_distance.avg))
        for key, val in ocr_metrics.items():
            logger.info(f"{key}: {val:.4f}")

        return {
            "token_acc": eval_metric.mean(),
            "batch_edit_dis": batch_edit_distance.avg,
            "final_edit_dist": ocr_metrics.get("edit_dist", 0.0),  # separate naming
            "bleu": ocr_metrics.get("bleu", 0.0)
        }
    ...
    def init_model(self, config):
        ...
        pretrained_swin_name = model_args["pretrained_model_name_or_path"] 
        swin_model = HybridResNetSwinEncoder(pretrained_swin_name)

        mbart_decoder = MBartForCausalLM.from_pretrained(pretrained_model_name_or_path)

        # Build the multi-bucket processor
        image_processor = NougatLaTexProcessor(
            size=None,
        ...
        config = VisionEncoderDecoderConfig.from_encoder_decoder_configs(swin_model.config, mbart_decoder.config)
        ...
        # Ensure the decoder's vocabulary size is set correctly
        config.decoder.vocab_size = mbart_decoder.config.vocab_size
        ...
        print(model)           # <-- Add this line to print the entire architecture
    ...
    def _print_step_log(self, epoch, global_step, global_eval_step, loss_meter, norm_meter, batch_time, ni, **kwargs):
        ...
        if global_step > 0 and self.args.trainer.save_step_freq > 0 and self.args.device.is_master and global_step % self.args.trainer.save_step_freq == 0:
            ...
            checkpoint_name = "{}_epoch{}_step{}_lr{:e}_avg_loss{:.5f}_token_acc{:.5f}_final_edit_dist{:.5f}_bleu{:.5f}.pth".format(
                self.experiment_name, epoch, global_step, current_lr,
                loss_meter.avg, result["token_acc"], result.get("final_edit_dist", 0.0), result.get("bleu", 0.0))
